...
def test_mongo_platform(monkeypatch: MonkeyPatch) -> None:
    ...
    with raises(OSError) as excinfo:
        # pylint: disable=expression-not-assigned
        MongodBinary(path).current_platform
    ...
    assert MongodBinary(path).current_platform == 'linux-x86_64-ubuntu1604'

    monkeypatch.setattr("distro.version", lambda: "18.04")
    monkeypatch.setattr("distro.major_version", lambda: "18")
    assert MongodBinary(path).current_platform == 'linux-x86_64-ubuntu1804'

    monkeypatch.setattr("distro.version", lambda: "20.04")
    monkeypatch.setattr("distro.major_version", lambda: "20")
    assert MongodBinary(path).current_platform == 'linux-x86_64-ubuntu2004'
    ...
    assert MongodBinary(path).current_platform == 'linux-x86_64-debian92'

    monkeypatch.setattr("distro.version", lambda: "10.0")
    monkeypatch.setattr("distro.major_version", lambda: "10")
    assert MongodBinary(path).current_platform == 'linux-x86_64-debian10'
    ...
    assert MongodBinary(path).current_platform == 'macos-x86_64'

    # WINDOWS

    monkeypatch.setattr("platform.system", lambda: "Windows")
    assert MongodBinary(path).current_platform == 'windows-x86_64'
